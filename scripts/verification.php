<?php
include($_SERVER['DOCUMENT_ROOT'] . '/CabinetMedical/scripts/connexion.php'); 

if(isset($_POST['id']) && isset($_POST['mdp'])) {

  if(preg_match('`([\-_.,;:\|]+)`', $_POST['id'])) {                        // Ces caractères ne sont pas utilisables pour renseigner l'identifiant

    header('Location: /CabinetMedical/index.php?erreur=2');                 // redirection vers index.php, via header.php affiche "Caractères spéciaux interdit"

  } elseif (preg_match('`([\-_.,;:\|]+)`', $_POST['id'])) {                 // Ces caractères ne sont pas utilisables pour renseigner le mot de passe

    header('Location: /CabinetMedical/index.php?erreur=2');                 // redirection vers index.php, via header.php affiche "Caractères spéciaux interdit"

  } else {
    
      if ($_POST['id'] == "salu") {                                         // verification de l'identifiant

        if ($_POST['mdp'] == "sava") {                                      // verification du mot de passe

          $_SESSION['connexion'] = 'oui';                                   // connexion réussite
          header('Location: /CabinetMedical/pages/usagers/rechercher.php'); // redirection vers l'accueil 

        } else {
          header('Location: /CabinetMedical/index.php?erreur=1');           // redirection vers index.php, via header.php affiche "Utilisateur ou mot de passe incorrect"
        }
      } else {
          header('Location: /CabinetMedical/index.php?erreur=1');           // redirection vers index.php, via header.php affiche "Utilisateur ou mot de passe incorrect"
      }
  }
} 
?>